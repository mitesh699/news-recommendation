...
import { getSavedArticles, removeSavedArticle as removeArticle, getReadingHistory, clearReadingHistory as clearHistory } from '../services/userDataService';
...
const UserProfile = () => {
  ...
  // Load user data and handle URL tab parameters
  useEffect(() => {
    if (currentUser) {
      setUserPreferences({
    ...
    if (tabParam && ['general', 'preferences', 'saved', 'history', 'security', 'privacy'].includes(tabParam)) {
  ...
  const removeSavedArticle = (articleId) => {
    const result = removeArticle(articleId);
    if (result.success) {
  ...
  const clearReadingHistory = () => {
    ...
    if (result.success) {
      ...
      console.error('Failed to clear reading history:', result.message);
    }
  };
